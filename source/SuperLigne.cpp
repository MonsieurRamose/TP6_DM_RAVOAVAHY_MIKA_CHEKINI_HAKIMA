...
SuperLigne::SuperLigne(Terminal *orig, Terminal *dest, int freq)
    ...
    switch (ajout) {
        case 0:
            std::cout << "Creation d'une liaison entre " << origine->getNom() << " et " << destination->getNom()
                      << std::endl;
        case -1:
            std::cout << " Le terminal: " << origine->getNom() << " est deja relié à " << destination->getNom()
                      << std::endl;
            break;
        case -2:
            std::cout << " Le terminal: " << origine->getNom() << " a atteint sa capacité maximum de liaison "
                      << std::endl;
            break;
        case -3:
            std::cout << " Le terminal " << origine->getNom()
                      << " n'est relié qu'avec des gares ou des hubs multimodal " << std::endl;
            break;
        case -4:
...
void SuperLigne::afficher() const {
    std::cout << " reliant " << origine->getNom() << " et " << destination->getNom() << ", frequence: " << frequence
              << " passagers/jour" << std::endl;
}


bool SuperLigne::operator==(SuperLigne *s) {
    return (this->origine == s->getOrigine()) && (this->destination == s->getDestination()) &&
           (this->frequence == s->getFrequence());
}
