...
AeroportRegional::AeroportRegional(std::string _nom, double lat, double lng)
        : Terminal(_nom, lat, lng, 0), MAX_LIAISON(1) {
    std::cout << "Creation de l'aeroport regional: " << this->nom << std::endl;
    this->afficher();
}
...
int AeroportRegional::ajouterLiaison(Terminal *terminal) {
    if (!terminal->estUneGare()) {
        if (liaisons.empty()) {
            ...
            return 1; // liaison ajoutée
        } else {
            return 0; // Un areoport régional est relié qu'avec un seul autre aeroport
        }
    } else {
        return -1; //un aeroport régional n est relié qu avec des aeroports
    }
